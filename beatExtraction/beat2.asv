function [b,t,onsetenv,oesr,D,cumscore] = beat2(d,sr,startbpm,tightness,doplot)
...
%   startbpm    specifies the target tempo.  
%               If it is a two-element vector, 
%               it is taken as the mode of a tempo search window, with 
...
% I do not ue startbpm. Hence temposd = 0
...
if sr < 2000
  onsetenv = d;
  oesr = sr;            % Onset Envelope Sampling Rate
...
if startbpm == 0 | temposd > 0 | length(onsetenv) == 0
  ...
  % temporary override of startbpm
  % choosing max
  % 2015-10-27
  startbpm = max(t(1), t(2));
